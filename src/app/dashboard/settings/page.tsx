...
/* ── Tabs ── */
const TABS = ["Account", "Notifications", "Privacy", "Billing", "Integrations", "Domain"] as const;
type Tab = typeof TABS[number];

/* ── Domain tab sub-components ── */
...
export default function SettingsPage() {
  ...
  function saveDomain() {
    setDomainSaved(true);
    setTimeout(() => setDomainSaved(false), 2200);
  }
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-6">
      ...
      {tab === "Domain" && (
        <>
          ...
          {domainChoice === null && (
            <>
              <div className="text-center px-2">
                <h2 className="font-sans text-base font-semibold text-[var(--fg)]">Choose your domain</h2>
                <p className="font-sans text-xs text-[var(--fg-muted)] mt-1.5 max-w-sm mx-auto">
                  Your domain is the address where clients find your portfolio, links page, and delivery galleries.
                </p>
              </div>
          ...
          {/* ── Step 2: custom domain input (only shown when custom chosen) ── */}
          {domainChoice === "custom" && (
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden">
              <div className="px-6 py-4 border-b border-[var(--border)] flex items-center justify-between">
                <div>
          ...
          {/* ── Step 2 (free): confirmation banner ── */}
          {domainChoice === "free" && (
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden">
              <div className="px-6 py-4 border-b border-[var(--border)] flex items-center justify-between">
                <div>
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden">
              <div className="px-6 py-4 border-b border-[var(--border)]">
                ...
                <p className="font-sans text-xs text-[var(--fg-muted)] mt-0.5">
                  Control the URL path for each of your public pages under <span className="font-mono text-[var(--fg)]">{activeDomain || freeSubdomain}</span>
                </p>
              </div>
              <div className="divide-y divide-[var(--border-subtle)]">
                <RouteRow
                  ...
                  prefix={`${activeDomain || freeSubdomain}/`}
                  slug=""
                  locked
                />
                <RouteRow
                ...
                {[
                  ...
                  <div key={label} className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-[11px] text-[var(--fg)]">
                      {activeDomain || freeSubdomain}{path}
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className={`font-sans text-sm font-semibold px-5 py-2 rounded-lg transition-all ${
                  domainSaved ? "bg-green-500 text-white" : "bg-yellow text-[#111] hover:bg-yellow-dark"
                }`}
              >
                {domainSaved ? "Saved" : "Save changes"}
              </button>
            </div>
          )}
        </>
      ...
      {/* Save row — not shown on Billing, Integrations, or Domain */}
      {tab !== "Billing" && tab !== "Integrations" && tab !== "Domain" && (
        <div className="flex justify-end gap-3">
